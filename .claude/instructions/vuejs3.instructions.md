...
Instructions for building high-quality VueJS 3 applications with the Composition API and modern best practices.
...
### JavaScript Patterns
- Use `<script setup>` with `defineProps` (object syntax) and `defineEmits`
- Use JSDoc comments for documenting prop types, function signatures, and return values
- Validate props with runtime validators in `defineProps` where needed
- Use descriptive variable names and consistent data shapes
- Document complex composable inputs/outputs with JSDoc `@param` and `@returns`
...
- Validate props with runtime checks using object syntax in `defineProps`
...
2. Initialize Vite project with Vue 3
